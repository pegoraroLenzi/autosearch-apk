...
6. **Armazenamento duplo com arquivo point-in-time perpétuo** (decisão do Ponto 4): bruto no data lake (reprocessável) e curado no banco relacional + índice vetorial para busca semântica pelos agentes. **Todo documento coletado é arquivado com timestamp de captura e nunca descartado** — inclusive o que o classificador julga irrelevante hoje. É o ativo que torna possível, em 2–3 anos, backtestar as fontes que não têm histórico comercial (diários municipais, imprensa regional, reviews): não dá para arquivar retroativamente. Duas extensões (mitigação das fraquezas §14.1, decididas):
   ...
   - **Blindagem do arquivo — em dois regimes** (corrigido no Red-team #2, achado 3: WORM total sobre documentos de terceiros colidiria com o direito de exclusão da LGPD, Art. 18): (a) a **trilha de auditoria própria** — decisões, ordens, teses, sinais e a **cadeia de hashes** dos documentos — é gravada com **object lock/WORM** (imutável: é dado nosso, sem dado pessoal de terceiro); (b) o **acervo de documentos coletados** fica em bucket com **versionamento, trava administrativa e replicação para segunda região/provedor** — protegido contra acidente e adulteração, mas com um **procedimento de exclusão LGPD documentado**: pedido legítimo do titular ou ordem judicial remove o conteúdo deixando um *tombstone* (hash, metadados, motivo e data da remoção), preservando a integridade probatória da cadeia sem reter o dado. **Teste de restore trimestral** com verificação de hashes cobre os dois regimes. O arquivo é o único ativo do fundo que não se reconstrói; sua proteção nasce junto com ele, na semana 1.
...
- **Modo de construção (Red-team #1, achado 2 — decidido pelo gestor):** durante a fase de construção/validação, os peers têm **coleta e arquivo integrais** (o dado não espera), mas o ciclo diário de agentes roda só para o universo investível; a análise espelhada dos peers roda **no comparativo trimestral e sob evento relevante** (classificador §3.2). O desenho cheio (ciclo diário para peers) volta quando os gates liberarem orçamento maior — a mudança é configuração.
- **Dois tipos de peer:**
...
Análises e posições são **separadas e estruturadas por horizonte temporal**, em cinco livros (quatro de análise profunda + a camada sistemática). Cada livro tem pergunta própria, sinais dominantes, cadência de revisão e orçamento de risco — e uma tese pertence a exatamente um livro:

> **Livro de curtíssimo prazo (1–10 dias): eliminado do escopo** (Red-team #1, achado 8, decidido pelo gestor). Apostar em sobre-reação a evento exige dado intraday e execução imediata — incompatível com o ciclo EOD, com a execução paciente do §6.4 (ordem limitada passiva em evento sofre seleção adversa) e com a proibição de day trade do mandato. Eventos de curtíssimo prazo cumprem o papel que a regra 2 abaixo já define: **ajustam timing e tamanho** das posições dos demais livros. O livro pode ser reativado (via `policy_config`) quando existirem dados intraday e ponte B3 reais.

| Livro | Janela | Pergunta que responde | Sinais dominantes | Cadência de revisão | Orçamento de risco indicativo* |
|---|---|---|---|---|---|
| **Curto prazo** | semanas a 1 trimestre | O próximo resultado confirma ou surpreende? | Promessa×entrega do trimestre, revisões de expectativa, momentum, fluxo de notícias, sinais rápidos de Pessoas (layoffs, êxodo), reação a eventos de alto impacto | Diária | 5–10% |
...
| **Camada sistemática (breadth)** | rebalanceamento mensal | Os fatores/small caps pagam no agregado? | **Modo configurável em `policy_config`** (nunca código): `etf` \| `nomes_próprios` \| `desativada`. **Por ora: ETF de small caps (ex.: SMAL11)** — executável em qualquer capital, sem contaminar a isenção de IR das ações | Mensal (rebalance) | 10–20% |
...
**Nota sobre a camada sistemática (cobertura em dois níveis — Ponto 3 da revisão; modo ajustado no Red-team #1, achado 7):** é a resposta a Grinold-Kahn (IR ≈ IC × √breadth) — compra amplitude barata onde profundidade não é necessária, enquanto os demais livros concentram a profundidade. Por ser regra-baseada, **não passa pelos agentes nem exige os pré-requisitos do §4.1**; tem **uma tese única de estratégia** versionada e obedece ao mandato (long-only). **Com capital PF pequeno, a versão de 100+ nomes próprios é inexecutável** (tickets abaixo do lote-padrão; rebalance mensal estoura o limite de R$ 20 mil de vendas e contamina a isenção de IR de todos os livros; fricção por nome come o prêmio de fator) — por isso o modo inicial é **ETF**, e a versão `nomes_próprios` (100+ ativos, fatores calculados em casa) fica **condicionada a um piso de PL definido no mandato após o Gate 2**. A troca de modo é configuração, com a tese de estratégia reescrita e versionada a cada mudança.
...
6. **Re-underwriting cego do núcleo (Ponto 10; simplificado no Red-team #2, achado 6 — semestral e automático):** a cada 6 meses, cada posição do livro de retenção é reavaliada do zero por uma rodada com **montagem de contexto cega** — e a cegueira é engenharia de contexto deliberada, não um dado: o contexto padrão do §4.7 injeta a Base Específica com teses, posições e casos da própria empresa, então a rodada cega **exclui explicitamente** esses elementos (fica ficha de consistência, dossiê, dados e documentos). Sem rito de comitê próprio: a reavaliação roda automaticamente, e **divergência entre a tese cega e a posição vira item normal da fila de decisão (§5.3), com ônus da prova invertido** — quem quer manter precisa provar. Antídoto ao viés de dotação onde ele realmente mora: no contexto acumulado do sistema e na decisão humana; o ciclo coincide com a revisão semestral de fontes (§10 item 5).
...
1. **Comitê (geração de evidência).** O **Agente Gestor (PM)** conduz a rodada de debate para ativos com sinais fortes ou divergentes (padrão adversarial — "advogado do diabo" tenta derrubar a tese) e produz a **tese escrita**: o que comprar/vender, por quê, evidências, gatilho de saída, risco que invalida, cenários do Planejamento Estratégico em que a tese sobrevive. **Três campos obrigatórios** (melhoria decidida pelo gestor — separam análise de aposta):
   - **Percepção variante — definida operacionalmente** (Red-team #1, achado 3; flexibilizada no Red-team #2, achado 5): em papéis sem cobertura não existe consenso de analistas — então **o consenso é o próprio preço**. O campo exige a comparação explícita: *o que o preço atual assume vs. o que nossa análise sustenta, e com base em qual informação que o preço ainda não reflete*. A ancoragem quantitativa das expectativas implícitas usa **a ferramenta adequada ao caso**: reverse-DCF quando o FCF é positivo e estável; **valor patrimonial/de liquidação, múltiplos normalizados (EV/EBITDA histórico) ou custo de reposição** quando o FCF é negativo ou errático (turnarounds, reestruturações — frequentes no nosso território). O verificador exige **que exista uma ancoragem coerente e declarada**, não uma fórmula específica; narrativa sem ancoragem nenhuma é reprovada.
   - **Catalisador com prazo — evento mecânico verificável** (idem): divulgação de resultado, dividendo/JCP, recompra anunciada, OPA, entrada/saída de índice, vencimento de contrato/concessão, decisão judicial com data. **"O mercado vai perceber" não é catalisador** — em papel negligenciado, ninguém está olhando para perceber; o verificador rejeita catalisadores não-mecânicos. Estar certo cedo demais é estar errado.
   ...
   - **Heterogeneidade de modelos nos papéis adversariais:** o refutador e ao menos um verificador rodam em **família de LLM diferente** da do proponente — em ensembles, o ganho vem de diversidade de erros; cético da mesma família compra os mesmos vieses e o debate vira teatro. **Na fase de construção/validação** (Red-team #1, achado 2 — decidido pelo gestor): a heterogeneidade é aplicada a **teses acima do limiar de alçada e promoções ao núcleo** (limiar em `policy_config`); o dia a dia roda numa família só, e o desenho cheio volta quando os gates liberarem orçamento maior.
...
2. **Camada de calibração.** Os scores brutos de cada agente são mapeados para probabilidades por **regressão isotônica** ajustada nos resultados realizados, com **Brier score e curvas de confiabilidade monitorados por agente e por setor**. **Shrinkage em direção ao prior de pesos iguais — nunca a zero** (Red-team #2, achado 2): agente sem histórico suficiente encolhe para o peso do cold start, não para a irrelevância; agentes de baixa frequência (Fundamentalista, Planejamento Estratégico — poucos sinais/ano por natureza) têm **piso estrutural de peso em `policy_config`** e são avaliados no horizonte deles. Sem isso, o viés de amostragem transformaria o meta-modelo num modelo de sentimento/momentum de curto prazo — violando o mandato, que exige os sinais qualitativos representados.
...
- **Modelo de risco de fatores (Ponto 5; estimação robusta adicionada no Red-team #2, achado 7):** cada posição é decomposta em exposições a fatores observáveis — mercado (Ibov), juros (DI), câmbio (BRL), commodities, valor/momentum/qualidade — por regressão das ações nos fatores (Barra simplificado, sem licença paga). **Em small caps ilíquidas, OLS diário simples produz betas falsos** (negociação assíncrona, dias sem negócio, saltos de bid-ask) — a estimação usa **correção de assincronia (Dimson/Scholes-Williams), retornos semanais/janelas longas e shrinkage bayesiano em direção ao beta setorial**, e os limites de fator têm **histerese (buffer de entrada/saída)** para que ruído de estimação não ligue e desligue o modo só-redução. O motor impõe **limites sobre as exposições líquidas do portfólio a cada fator**, além dos limites por nome/setor/livro: dez teses "independentes" que são a mesma aposta em juros ficam visíveis e limitadas. É a defesa estrutural contra o modo de falha do LTCM.
...
- **Stops: intraday quando houver dado, honestos quando não houver** (ajustado no Red-team #1, achado 8): o objetivo é executar stops no disparo, sem esperar o fechamento — mas isso exige cotação intraday e ponte de execução, que **não existem no stack gratuito-EOD da fase de construção**. Regra por fase: enquanto o dado for EOD, os stops são registrados como **ordens stop na corretora** (quando a ponte suportar) ou avaliados no ciclo diário — e essa limitação fica explícita na tese e no dimensionamento (posição menor onde a defesa é mais lenta). **Conflito stop × day trade resolvido a favor da proteção:** stop pode executar no mesmo dia da compra (aceitando o enquadramento tributário pontual — imposto é menor que perda descontrolada); o motor evita o inverso (recomprar no dia da venda).
...
**Estratégia de validação de execução (corrigida no Red-team #1, achado 1 — a rampa deve validar o mercado onde o dinheiro vai operar):**
1. **Paper trading da carteira B3 em simulador próprio** (`B3PaperBroker` atrás do `BrokerAdapter`): fills modelados com dados B3 reais (liquidez, spread estimado por papel, participação limitada) — é isso que o Gate 2 avalia.
2. **Alpaca como laboratório opcional de integração**: valida ordem→fill→reconciliação contra uma API real de corretora, sem fingir que valida a estratégia.
3. **Ponte B3 real (MT5/Cedro) construída e testada durante o Gate 2**, rodando em modo espelho (recebe as mesmas ordens do simulador, sem executar); **"ponte B3 validada em modo espelho" é critério de entrada do Gate 3** — o capital real nunca estreia uma ponte nova.
...
| 10:00–17:00 | Execução paciente por ordens limitadas dentro da faixa de preço de cada tese (§6.4), com participação limitada no volume; monitor de eventos reage a fatos relevantes novos; stops conforme a regra por fase do motor de risco (§5) |
...
1. **Replay histórico — só onde há point-in-time real, e com escopo honesto sobre agentes** (Ponto 4 + Red-team #1, achado 4): o backtest walk-forward roda apenas sobre fontes com arquivo histórico íntegro (preços, fundamentos CVM, fatos relevantes). **A performance medida por backtest é a da camada sistemática e da mecânica do motor de risco** — para os agentes LLM, o replay é apenas **validação funcional** (o fluxo roda, as invariantes seguram), nunca métrica de performance: instrução de prompt não remove a memória paramétrica do modelo (ele *conhece* o desfecho de cada empresa até seu cutoff — o eval anacrônico pega citações, não convicção contaminada), e replay de anos de comitê multi-agente não cabe em orçamento algum. A performance da parte com agentes é validada **prospectivamente** (item 2), com critérios numéricos e prazos próprios. Fontes sem point-in-time (imprensa regional, diários, reviews, fornecedores) não entram no backtest.
...
| Item | Fase de construção (teto R$ 1.000/mês) | Pós-gates (escala com universo e capital) |
|---|---|---|
| APIs de LLM | ~US$ 120–180 (batch + cache obrigatórios, conta abaixo) | US$ 300 – 2.000 |
| Market data | US$ 0 (EOD gratuito) | US$ 0 – 500 (intraday se ativar stops/ponte) |
| Dados LinkedIn/Glassdoor licenciados | US$ 0 (Fase 3, só pós-Gate 2) | US$ 500 – 5.000 |
| Notícias/research | US$ 0 (fontes gratuitas + GDELT) | US$ 0 – 500 |
| Infraestrutura (cloud) | US$ 10 – 50 (mínima, storage barato) | US$ 100 – 400 |
...
**Universo inicial mínimo viável (mitigação §14.1; escopo derivado do orçamento de 15h/semana do mandato — Red-team #1, achado 6):** a partida é com **2 setores e ~10 empresas + 2–3 âncoras** (os 2 setores escolhidos entre os 4 candidatos no screening da semana 1, pelos dados); a expansão para os 4 setores e 15–20 empresas acontece via `universe_config` conforme os dossiês ficam prontos e as **horas reais do gestor couberem no orçamento** (métrica acompanhada). Seleção pelos **seis filtros objetivos** (decisão do gestor): (1) negligência analítica — cobertura de sell-side baixa (≤ ~5 analistas); (2) pegada física/regional identificável — para as fontes exóticas terem o que dizer; (3) liquidez mínima compatível com a regra de saída (≤ 5 pregões a ≤ 10% do ADV); (4) dossiê setorial viável com dados públicos ricos; (5) diversidade de fatores entre os setores; (6) exclusões da fase 1: bancos/seguradoras, empresas em recuperação judicial, estatais com preço dominado por política.
...
**Conta de viabilidade do teto (hipóteses registradas, a confirmar em produção; ajustada ao escopo de 2 setores):** ~10 empresas em ciclo diário + peers/fornecedores em coleta-e-arquivo (análise só trimestral/por evento) + 2–3 âncoras; classificador de relevância em modelo econômico (classe Haiku) processando ~1.000–1.500 documentos/dia via batch ≈ US$ 1,5/dia; análise profunda com **cadência adaptativa** — ~6–8 análises/dia × 6 agentes com a Base Fixa em prompt cache (§4.7) ≈ US$ 3–4/dia; comitê/debate para 1–2 ativos/dia (heterogeneidade só em alçada alta) ≈ US$ 1–1,5/dia. Total ≈ US$ 6–7/dia útil ≈ **R$ 400–700/mês com batch API** — o teto fecha com margem real. Batch + cache não são otimização opcional, são requisito de projeto; estouro sustentado reduz cadência (não qualidade por análise) e sobe ao gestor no relatório mensal. A expansão para 4 setores/20 empresas refaz esta conta antes de acontecer.
