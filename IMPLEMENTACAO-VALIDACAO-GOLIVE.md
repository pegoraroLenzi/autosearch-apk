...
- **Semana 1, antes de tudo: ligar o arquivo.** Scripts mínimos de coleta+arquivo point-in-time (S3/Parquet com timestamp de captura) das fontes efêmeras do universo inicial (sites de RI, diários, notícias regionais) rodando antes de qualquer outro componente — o fosso e o futuro backtest começam a contar do primeiro dia do projeto. **A blindagem nasce junto** (§3.2 item 6, dois regimes): WORM/object lock **só na trilha de auditoria própria e na cadeia de hashes**; o acervo de documentos coletados em bucket versionado com trava administrativa + replicação + **procedimento de exclusão LGPD com tombstone** — e teste de restore trimestral cobrindo os dois.
...
- Coletores restantes: notícias **regionais** (mapa de sede/operações), **portais oficiais de governo** nas 3 esferas (federal direto; estadual/municipal via **Querido Diário** + raspadores próprios onde necessário), **canais oficiais das companhias** (site/RI com diff), **judicial em escopo realista para a fase 1** (Red-team #2, achado 4): somente **APIs oficiais gratuitas** — API pública do DataJud e Comunica/DJEN por CNPJ (empresa e peers) — **cruzadas com as provisões e contingências das notas explicativas** (que a CVM já entrega estruturado); **raspagem direta de portais de tribunais fica fora da fase 1** (anti-bot/CAPTCHA tornam a manutenção incompatível com o teto), e agregador comercial só entra com orçamento e atribuição que o justifiquem; **radar de tecnologia/inovação** (INPI/USPTO, Crunchbase).
...
- **Critérios de aprovação (definidos antes de rodar)** — sugestão inicial, a calibrar:
  - Camada sistemática **conforme o modo ativo** (Red-team #2, achado 1): em modo `etf` (o atual), o ETF passivo não gera alpha por construção — o gate valida a **mecânica**: regra de rebalance executada corretamente no replay, orçamento do livro respeitado, e contribuição de risco do sleeve compatível com o stress e o drawdown do portfólio (um DD histórico de 40–50% do ETF sobre 10–20% do PL ≈ 4–10% de impacto — dentro do mandato). O critério de **Sharpe fora-da-amostra ≥ 0,8** aplica-se apenas se/quando o modo `nomes_próprios` for ativado.
...
- [ ] **Blindagem do arquivo point-in-time verificada** (dois regimes do §3.2): WORM ativo na trilha de auditoria e cadeia de hashes; acervo de documentos com versionamento/trava/replicação em dia; procedimento de exclusão LGPD testado (tombstone preservando integridade); último teste de restore trimestral passando.
