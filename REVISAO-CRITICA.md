...
> **Status de tratamento:** ponto 6 atendido (`MANDATO.md`); pontos 1–2 atendidos (§5.2 do projeto — meta-modelo com features qualitativas obrigatórias, cold start por pesos iguais + shrinkage, conflito assimétrico); ponto 3 atendido com decisão do gestor (§4.2 e §5.1 — cobertura em dois níveis aprovada; o histórico de 10 anos vira **contexto de compreensão, não fator determinante**: transparência via Ficha de Consistência de Dados no lugar de bloqueio ou modulação mecânica; IPOs entram com coleta pré-IPO obrigatória); ponto 4 atendido (arquivo point-in-time perpétuo desde o dia 1, pré-registro obrigatório de previsões falsificáveis em `predictions`, paper trading estendido para 6 meses com track record prospectivo como critério do Gate 2 — linha do tempo total ~12 meses); ponto 5 atendido (modelo de fatores com limites líquidos + stress diário com gatilho de só-redução); ponto 7 atendido (sprints re-sequenciados — dados nos Sprints 1–4, agentes a partir do 5; Querido Diário/DataJud/dados abertos CVM adotados com raspadores próprios onde necessário; golden set de entity linking como primeiro eval; linha do tempo ~13 meses); ponto 8 atendido (refutador e ≥1 verificador em família de LLM diferente + verificador determinístico não-LLM bloqueante por tese); ponto 9 atendido (atribuição por fonte via ablação, morte em 2 ciclos com notificação obrigatória ao gestor, TCO por empresa no relatório mensal); ponto 10 atendido (re-underwriting cego semestral do núcleo com ônus da prova invertido — §5.1 regra 6); ponto 11 atendido (§5.3 do projeto — SLA de 48h com default de não-execução, modo preservação em ausência, orçamento de atenção de 5 itens/dia). **Todos os 11 pontos desta revisão foram decididos pelo gestor e incorporados aos documentos do projeto.**
>
